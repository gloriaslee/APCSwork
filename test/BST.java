...
public class BST
{
  ...
  {
    return search( target, _root, null );
  }
  TreeNode[] search( int target, TreeNode currNode, TreeNode parent )
  {
    ...
    if ( currNode==null || currNode.getValue()==target ){
      ...
      return search( target, currNode.getLeft(),currNode );
    else if ( target > currNode.getValue() )
      return search( target, currNode.getRight(),currNode );
    else
      return ret; //to get past compiler
  }
...
public void remove(int target){
  TreeNode[] returned = search(target);
  if ( returned[0] != null)
    remove(returned[0],returned[1]);
  else
    System.out.println("Target not in tree");
}

public void remove(TreeNode node, TreeNode parent){
  if ( node.getLeft() == null && node.getRight() == null ){
    System.out.println("childless");
    if(node.equals(parent.getLeft())){
      parent._lt = null;
    }
    else{parent._rt = null;}
    
  } // no childs
  else if(node.getLeft()==null && node.getRight() != null){
    System.out.println("righty");
    if(node.getValue()==_root.getValue()){
      System.out.println("root");
      _root = node.getRight();
    }
    else if (node.equals(parent.getLeft())){
      parent._lt = node.getRight();
    }
  else{parent._rt = node.getLeft();}
